...
### Fixed
- **IBKR auto-allocation silently failing on nightly import** — Default allocations stored with camelCase `portfolioId` key but validated against snake_case `portfolio_id`, causing `process_transaction_allocation` to return `success=False` with no log output
  - Convert `defaultAllocations` from camelCase to snake_case before persisting to DB in the config save endpoint
  - Normalize keys when loading allocations in the automated import task, so existing configs with camelCase data work without a re-save
  - Add `WARNING` log when auto-allocation returns `success=False` (previously silent — only exceptions were logged)

## [1.6.1] - 2026-03-03

### Fixed
- **Apple device favicon support** — Safari/iOS doesn't support SVG favicons, causing the icon to not display on Apple devices
  - Added `apple-touch-icon.png` (180x180) for iOS home screen and tabs
  - Added PNG fallback favicon for browsers without SVG support
  - Updated `manifest.json` with 192x192 and 512x512 PNG icon entries
  - Added `copy-webpack-plugin` to copy `public/` assets to build output
