...
dividend_model = ns.model('Dividend', {
    ...
    'ex_dividend_date': fields.String(required=True, description='Ex-dividend date (YYYY-MM-DD)'),
    ...
    'reinvestment_status': fields.String(required=True, description='Reinvestment status',
                                         enum=['pending', 'completed', 'partial']),
    'buy_order_date': fields.String(description='Buy order date for reinvestment')
})

dividend_create_model = ns.model('DividendCreate', {
    'portfolio_fund_id': fields.String(required=True, description='Portfolio-Fund relationship ID'),
    'record_date': fields.String(required=True, description='Record date (YYYY-MM-DD)', example='2024-01-15'),
    'ex_dividend_date': fields.String(required=True, description='Ex-dividend date (YYYY-MM-DD)', example='2024-01-10'),
    'dividend_per_share': fields.Float(required=True, description='Dividend per share', example=0.50),
    'buy_order_date': fields.String(description='Buy order date for stock dividends (YYYY-MM-DD)'),
    'reinvestment_shares': fields.Float(description='Shares from reinvestment (for stock dividends)'),
    'reinvestment_price': fields.Float(description='Price per share for reinvestment')
})

dividend_update_model = ns.model('DividendUpdate', {
...
class Dividend(Resource):
    ...
    def get(self, dividend_id):
        ...
        try:
            ...
            logger.log(
                level=LogLevel.ERROR,
                category=LogCategory.DIVIDEND,
                message=f"Error retrieving dividend: {str(e)}",
    ...
    def put(self, dividend_id):
        ...
        try:
            data = request.json
            dividend = DividendService.update_dividend(dividend_id, data)
