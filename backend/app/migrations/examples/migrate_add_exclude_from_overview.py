...
def migrate(db_path):
    ...
    cursor.execute("SELECT COUNT(*) FROM pragma_table_info('portfolio') WHERE name='exclude_from_overview'")
    ...
    if not column_exists:
        ...
        try:
            # Add the column
            cursor.execute("ALTER TABLE portfolio ADD COLUMN exclude_from_overview BOOLEAN DEFAULT FALSE")
            
            # Set default value for existing rows
            cursor.execute("UPDATE portfolio SET exclude_from_overview = FALSE WHERE exclude_from_overview IS NULL")
            
            # If the old is_hidden column exists, migrate its values
            cursor.execute("SELECT COUNT(*) FROM pragma_table_info('portfolio') WHERE name='is_hidden'")
