...
const useChartData = (endpoint, params = {}, defaultZoomDays = 365) => {
    ...
    try {
      ...
      if (append) {
        setLoadedRange(prevRange => {
          ...
          return {
            start: startDate && new Date(startDate) < new Date(prevRange.start) ? startDate : prevRange.start,
  ...
  const checkAndLoadMoreData = useCallback((zoomState) => {
  ...
  const onZoomChange = useCallback((zoomState) => {
